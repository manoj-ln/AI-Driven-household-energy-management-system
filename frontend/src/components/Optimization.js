...
function Optimization() {
  ...
  useEffect(() => {
    async function loadReport() {
      ...
      if (data && typeof data === "object" && !data.error && data.peak_hour !== undefined) {
        setReport(data);
        return;
      }
      setLoadError(data?.error || "Could not load optimization report. Ensure the backend is running on port 8000.");
    }
    loadReport();
  }, []);
  ...
  return (
    <section style={{ marginBottom: "38px" }}>
      ...
      {safeReport ? (
        ...
        <p style={{ color: "#b45309", background: "#fff7eb", padding: "14px 16px", borderRadius: "12px" }}>{loadError}</p>
      ) : (
