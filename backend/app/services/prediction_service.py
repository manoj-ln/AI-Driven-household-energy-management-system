...
class PredictionService:
    ...
    DATASET_DEVICE_MAP = {
        ...
        "fan": ["Cooling_Fan_Ceiling", "Cooling_Fan_Table"],
        "refrigerator": ["Others_Refrigerator"],
        "air_conditioner": ["Cooling_AC", "Cooling_Cooler"],
        "heater": ["Heating_Room", "Heating_Water"],
        "oven": ["Kitchen_Induction"],
        "microwave": ["Kitchen_Microwave"],
        "light": ["Lighting_LED", "Lighting_Tube"],
        "tv": ["Electronics_TV"],
        "home_energy": [],
    }
    DEVICE_PROFILES = {
    ...
    @classmethod
    def _recent_records(cls, hours: int = 240) -> list[dict]:
        records = DatasetService.get_prediction_records(hours)
        if records:
            return records
        return [
    ...
    def _device_records(cls, device_key: str) -> list[dict]:
        records = DatasetService.get_prediction_records(400)
        if not records:
            return []

        normalized = cls._normalize_device_key(device_key)
        mapped_columns = cls.DATASET_DEVICE_MAP.get(normalized, [])
        matches = []
        for record in records:
            timestamp = record.get("timestamp")
            if not timestamp:
                continue
            if normalized == "home_energy":
                value = cls._reading_to_kwh(record)
            else:
                appliances = record.get("appliances", {})
                value = sum(float(appliances.get(column, 0.0)) for column in mapped_columns)
                if value <= 0:
                    continue
            matches.append(
                {
                    "timestamp": timestamp.isoformat() if hasattr(timestamp, "isoformat") else timestamp,
                    "energy_consumption": round(float(value), 6),
    ...
    def _load_models(cls):
        recent = cls._recent_records(240)
        hourly_avg = {}
        if recent:
    ...
    def get_available_models(cls):
        ...
        accuracy_source = {}
        preview_predictions = {}
        records = cls._recent_records(48)
    ...
    def predict_multi_step(cls, hours_ahead: int = 7) -> list:
        ...
        predictions = []
        latest_record_time = cls._parse_timestamp(records[-1]["timestamp"]) if records else datetime.now()
        current_time = latest_record_time or datetime.now()
